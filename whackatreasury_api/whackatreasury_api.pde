...
AudioSnippet win;
AudioSnippet fail;
AudioSnippet end, end1, end2, end3;
...
PImage defaultImage, readyImage, winImage;
int numSquares = 6;
...
int treasurySize = 16;
int deadGameCount = 0;
long gameTimeLimit = 240000; // 3 minutes
long endTime;
long startTime;
boolean isWon = false;
...
void setup() {
  ...
  end = minim.loadSnippet("sounds/Circus.mp3");
  end1 = minim.loadSnippet("sounds/Circus.mp3");
  end2 = minim.loadSnippet("sounds/Gong-short.mp3");
  end3 = minim.loadSnippet("sounds/Maple.mp3");
  timer = new Timer(1200); // display image in millis
  ...
  winImage = loadImage("extras/winning.png");
  textFont(font);
}
